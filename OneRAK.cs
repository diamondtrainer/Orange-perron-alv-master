...
namespace Orange_perron_chido
{
    public partial class OneRAK : Form
    {
        ...
        public OneRAK(List<List<string>> columnas, int index, Form1 anterior)
        {
            ...
            modelos = new Rules[K];
            int counter = 0;
            List<List<List<string>>> Experimental = new List<List<List<string>>>();
            while (cont < K)
            {
                if (cont == K - 1)
                {
        ...
        void KFolk (Rules[] reglas, List<string> target, List<List<string>> Tablas, int[] Finales)
        {
            int cont = 0;
            while (cont<K)
            {
                List<string> NOTTarget = NotTarget(reglas[cont], Tablas);
                if(cont==K-1)
                {
                    Finales[cont] = 1;
                }
